...
.. note::
    The panda robot environment contains two methods of controlling the robot: DIRECT control (Default)
    and PROXY based control. Initially, I abstracted all the panda control logic away in the
    'panda_gazebo' package and made it available through services. Later, however, I found
    that the service calls slowed down the control. I then added the DIRECT control method,
    which directly publishes the control commands on the controller ``command`` topic. This
control method made the training loop two times faster. Currently, the DIRECT mode is
    only available for the ``position`` and ``effort`` control. Other control methods
    like ``trajectory`` and ``end_effector`` control will use the PROXY based method.
"""  # noqa: E501
...
from openai_ros.exceptions import EePoseLookupError, EeRpyLookupError
from openai_ros.robot_gazebo_goal_env import RobotGazeboGoalEnv
...
class PandaEnv(RobotGazeboGoalEnv):
    ...
    def get_ee_pose(self):
        ...
        Raises:
            :obj:`openai_ros.errors.EePoseLookupError`: Error thrown when error occurred
                while trying to retrieve the EE pose using the 'get_ee_pose' service.
        """
        try:
    ...
    def get_ee_rpy(self):
        ...
        Raises:
            :obj:`openai_ros.errors.EeRpyLookupError`: Error thrown when error occurred
                while trying to retrieve the EE rpy rotation using the 'get_ee_pose'
                service.
        """
        try:
            # Retrieve EE pose using tf2
    ...
    def joints(self):
        ...
        .. important::
            ...
            first call. Please call the :method:`~PandaEnv.refresh_joints` method to
