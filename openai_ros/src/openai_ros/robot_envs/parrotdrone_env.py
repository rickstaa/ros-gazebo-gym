...
class ParrotDroneEnv(RobotGazeboEnv):
    ...
    def wait_for_height(self, heigh_value_to_check, smaller_than, epsilon, update_rate):
        ...
        while not rospy.is_shutdown():
            ...
            if smaller_than:
                ...
                rospy.logwarn(
                    "SMALLER THAN HEIGHT...current_height="
                    + str(current_height)
                    + "<="
                    + str(heigh_value_to_check)
                )
            else:
                takeoff_height_achieved = current_height >= heigh_value_to_check
                rospy.logwarn(
                    "BIGGER THAN HEIGHT...current_height="
                    + str(current_height)
                    + ">="
                    + str(heigh_value_to_check)
                )
    ...
    def move_base(
        self, linear_speed_vector, angular_speed, epsilon=0.05, update_rate=10
    ):
        """
