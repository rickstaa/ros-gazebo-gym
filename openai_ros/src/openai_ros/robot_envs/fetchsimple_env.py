...
class FetchSimpleEnv(RobotGazeboEnv):
    ...
    def get_joint_limits(self):
        return (
            self.move_fetch_object.joint_upper_limits,
            self.move_fetch_object.joint_lower_limits,
        )
...
class FetchSimpleMove(object):
    ...
    def wait_for_joints_to_get_there(self, desired_pos_array, error=0.2, timeout=3.0):
        ...
        while not are_equal and not is_timeout and not rospy.is_shutdown():
            ...
            are_equal = np.allclose(a=current_pos, b=desired_pos_array, atol=error)
    ...
    def move_all_joints(self, joints_pos_array, time_out=3.0, error=0.2):
        ...
        self.wait_for_joints_to_get_there(
            self.joint_array, error=error, timeout=time_out
        )
