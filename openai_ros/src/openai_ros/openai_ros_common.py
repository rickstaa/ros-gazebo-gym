...
def StartOpenAI_ROS_Environment(task_and_robot_environment_name):
    ...
    result = RegisterOpenAI_Ros_Env(
        task_env=task_and_robot_environment_name, max_episode_steps=10000
    )

    if result:
        rospy.logwarn(
            "Register of Task Env went OK, lets make the env..."
...
class ROSLauncher(object):
    def __init__(
        ...
        try:
            ...
            pkg_path = self.DownloadRepo(
                package_name=rospackage_name, ros_ws_abspath=ros_ws_abspath
            )
        ...
        if pkg_path:
            ...
            assert False, "No Package Path was found for ROS apckage ==>" + str(
                rospackage_name
            )
    ...
    def DownloadRepo(self, package_name, ros_ws_abspath):
        ...
        if package_name == "moving_cube_description":
            ...
            package_git.append("https://github.com/paulbovbel/frontier_exploration.git")
            ...
            or package_name == "cartpole_v0_training"
        ):
            package_git = ["https://bitbucket.org/theconstructcore/cart_pole.git"]

        elif (
            package_name == "legged_robots_sims"
            or package_name == "legged_robots_description"
            or package_name == "my_legged_robots_description"
            or package_name == "my_legged_robots_sims"
            or package_name == "my_hopper_training"
        ):
            package_git = ["https://bitbucket.org/theconstructcore/hopper.git"]

        elif (
            package_name == "iri_wam_description"
            or package_name == "iri_wam_gazebo"
            or package_name == "iri_wam_reproduce_trajectory"
            or package_name == "iri_wam_aff_demo"
        ):
            ...
            or package_name == "drone_demo"
            or package_name == "sjtu_drone"
            or package_name == "custom_teleop"
            or package_name == "ardrone_as"
        ):
