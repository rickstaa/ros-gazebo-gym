...
class TurtleBot2MazeEnv(turtlebot2_env.TurtleBot2Env):
    ...
    def _init_env_variables(self):
        ...
        self.publish_filtered_laser_scan(
            laser_original_data=laser_scan, new_filtered_laser_range=discretized_ranges
        )
    ...
    def _is_done(self, observations):
        ...
        if self._episode_done:
            rospy.logdebug(
                "TurtleBot2 is Too Close to wall==>" + str(self._episode_done)
            )
        else:
    ...
    def discretize_observation(self, data, new_ranges):
        ...
        self.publish_filtered_laser_scan(
            laser_original_data=data, new_filtered_laser_range=discretized_ranges
        )
    ...
    def publish_filtered_laser_scan(
        ...
        new_angle_incr = abs(
            laser_original_data.angle_max - laser_original_data.angle_min
        ) / len(new_filtered_laser_range)
