...
class ShadowTcGetBallEnv(shadow_tc_env.ShadowTcEnv):
    ...
    def _init_env_variables(self):
        ...
        self.previous_distance_from_ball = self.get_distance_from_point(
            self.ball_pose.position, tcp_pose.position
        )
    ...
    def _get_obs(self):
        ...
        self.finger_collided_dict = self.get_fingers_colision(
            object_collision_name="cricket_ball__link"
        )
        ...
        observation = [
            round(tcp_pose.position.x, self.dec_obs),
    ...
    def _is_done(self, observations):
        ...
        has_reached_the_ball = self.reached_ball(
            tcp_pos,
        ...
        rospy.logdebug(
            "Not bool_is_inside_workspace ?=" + str(not (bool_is_inside_workspace))
        )
    ...
    def _compute_reward(self, observations, done):
        ...
        distance_from_ball = self.get_distance_from_point(
            self.ball_pose.position, tcp_pos
        )
    ...
    def reached_ball(
        self, tcp_position, ball_position, minimum_distance, finguers_collided
    ):
        """
