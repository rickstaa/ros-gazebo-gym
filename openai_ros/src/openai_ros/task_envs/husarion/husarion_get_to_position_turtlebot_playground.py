...
class HusarionGetToPosTurtleBotPlayGroundEnv(husarion_env.HusarionEnv):
    def __init__(self):
        ...
        high_des_pos = numpy.array([self.work_space_x_max, self.work_space_y_max])
        low_des_pos = numpy.array([self.work_space_x_min, self.work_space_y_min])
    ...
    def _set_action(self, action):
        ...
        rospy.logdebug(
            "END Set Action ==>" + str(action) + ", ACTION=" + str(last_action)
        )
    ...
    def _get_obs(self):
        ...
        odometry_array = [round(x_position, 1), round(y_position, 1), round(yaw, 1)]
        ...
        desired_position = [
            round(self.desired_position.x, 1),
    ...
    def _is_done(self, observations):
        ...
        reached_des_pos = self.check_reached_desired_position(
            current_position, desired_position, self.precision_epsilon
        )
    ...
    def discretize_scan_observation(self, data, new_ranges):
            ...
            if i % mod == 0:
                if item == float("Inf") or numpy.isinf(item):
                    ...
                    if item > self.max_laser_value:
                        rospy.logwarn(
                            "Item Bigger Than MAX, CLAMPING=>"
                            + str(item)
                            + ", MAX="
                            + str(self.max_laser_value)
                        )
                        discretized_ranges.append(round(self.max_laser_value, 1))
                    elif item < self.min_laser_value:
                        rospy.logwarn(
                            "Item smaller Than MIN, CLAMPING=>"
                            + str(item)
                            + ", MIN="
                            + str(self.min_laser_value)
                        )
        ...
        self.publish_filtered_laser_scan(
            laser_original_data=data, new_filtered_laser_range=filtered_range
        )
    ...
    def check_husarion_has_crashed(self, laser_readings):
        ...
        for laser_distance in laser_readings:
            ...
            if laser_distance == self.min_laser_value:
                ...
                rospy.logerr(
                    "Value of laser shouldnt be lower than min==>"
                    + str(laser_distance)
                    + ", min="
                    + str(self.min_laser_value)
                )
                ...
                rospy.logerr(
                    "Value of laser shouldnt be higher than max==>"
                    + str(laser_distance)
                    + ", max="
                    + str(self.min_laser_value)
                )
    ...
    def check_reached_desired_position(
        self, current_position, desired_position, epsilon=0.1
    ):
        """
